...
const getSectionsInCourse = async (dept, course) => {
  ...
  const requiredFields = sectionsWithWaitListFiltered
    .map(({ instructors = '', _activity, _credits }) =>
      ({ instructor: instructors.instructor ? instructors.instructor._name : '', activty: _activity, credits: _credits }))
  return requiredFields
}

const writeHeader = async header => fswrite(filepath, header)

writeHeader(c.csvHeaders)

c.departments.forEach(async dept => {
  const courseObjs = await getCoursesInDept(dept, year, term)
  courseObjs.forEach(async ({ _key, _title }) => {
    const sections = await getSectionsInCourse(dept, _key)
